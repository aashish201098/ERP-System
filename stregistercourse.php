...
<?php
	...
	<nav class="navbar navbar-inverse">
	  <div class="container-fluid">
	    ...
	    <ul class="nav navbar-nav">
	      ...
	      <li class="active"><a href="stregistercourse.php">Register for Courses</a></li>
		<li class="dropdown">
	...
	<div class="container">
		<table class="table table-hover table-bordered">
			...
			<form class="form-horizontal" action="stregistercourse.php" method="post">
	<?php 
		include 'db.php';
		$sql = "select * from student where stid = '$usr'";
		$result = mysqli_query($conn,$sql);
		...
		$year = $row['year'];
		$deptid = $row['deptid'];
		$sql = "select * from course where year = '$year' and deptid = '$deptid'";
		$result = mysqli_query($conn,$sql);
		while($row = mysqli_fetch_array($result)){ ?>
			<tr>
...
<?php 
	if(isset($_POST['submit'])){
		...
		foreach ($courseid as $var) {
			$sql = "select * from registered where stid = '$usr' and courseid = '$var'";
			$result = mysqli_query($conn,$sql);
			if(mysqli_num_rows($result) > 0){

			}else{
				$sql = "insert into registered(stid,courseid) values('$usr','$var')";
